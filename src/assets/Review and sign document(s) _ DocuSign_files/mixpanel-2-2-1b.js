 ...
 */
        ...
        */
        ...
        function getElementsBySelector(selector) {
            ...
            for (var i = 0; i < tokens.length; i++) {
                ...
                var foundCount = 0;
                for (var h = 0; h < currentContext.length; h++) {
                    var elements = currentContext[h].getElementsByTagName(
                        tagName
                    );
                    for (var j = 0; j < elements.length; j++) {
                        found[foundCount++] = elements[j];
    ...
    _.info = {
        ...
        pageviewInfo: function(page) {
            return _.strip_empty_properties({
                mp_page: _.info.referringDomain(page),
                mp_referrer: _.info.referringDomain(document.referrer),
    ...
    DomTracker.prototype.track_callback = function(
        ...
        return function() {
            ...
            if (
                user_callback &&
                user_callback(timeout_occured, props) === false
            ) {
    ...
    LinkTracker.prototype.create_properties = function(properties, element) {
        var props = LinkTracker.superclass.create_properties.apply(
            this,
            arguments
        );
    ...
    LinkTracker.prototype.event_handler = function(evt, element, options) {
        options.new_tab =
            evt.which === 2 || evt.metaKey || element.target === "_blank";
    ...
    LinkTracker.prototype.after_track_handler = function(
        props,
        options,
        timeout_occured
    ) {
    ...
    FormTracker.prototype.after_track_handler = function(
        props,
        options,
        timeout_occured
    ) {
     ...
     * @param {number=} days
     */
    MixpanelCookie.prototype.register_once = function(
        props,
        default_value,
        days
    ) {
        ...
        this.register_once(
            {
                $initial_referring_domain:
                    _.info.referringDomain(referrer) || "$direct"
            },
            ""
        );
    };
    ...
    MixpanelCookie.prototype._get_or_create_queue = function(
        queue,
        default_val
    ) {
    ...
    MixpanelLib.prototype._init = function(token, config, name) {
        ...
        this.unregister("$initial_referrer");
        this.unregister("$referrer");
    };
     ...
     */
    MixpanelPeople.prototype.track_charge = function(
        amount,
        properties,
        callback
    ) {
    ...
    MixpanelPeople.prototype._flush = function(
        ...
        var _this = this,
            $set_queue = _.extend(
                {},
                this._mixpanel.cookie._get_queue(SET_ACTION)
            ),
            $set_once_queue = _.extend(
                {},
                this._mixpanel.cookie._get_queue(SET_ONCE_ACTION)
            ),
            $add_queue = _.extend(
                {},
                this._mixpanel.cookie._get_queue(ADD_ACTION)
            ),
        ...
        if (
            ...
            _this._mixpanel.cookie._pop_from_people_queue(
                SET_ACTION,
                $set_queue
            );
            this.set($set_queue, function(response, data) {
                ...
                if (response == 0) {
                    _this._mixpanel.cookie._add_to_people_queue(
                        SET_ACTION,
                        $set_queue
                    );
                }
    ...
    MixpanelCookie.prototype["properties"] =
        MixpanelCookie.prototype.properties;
    MixpanelCookie.prototype["update_search_keyword"] =
        MixpanelCookie.prototype.update_search_keyword;
    MixpanelCookie.prototype["update_referrer_info"] =
        MixpanelCookie.prototype.update_referrer_info;
    MixpanelCookie.prototype["get_cross_subdomain"] =
        MixpanelCookie.prototype.get_cross_subdomain;
    ...
    mixpanel["init"] = function(token, config, name) {
        if (name) {
            ...
            if (!mixpanel[name]) {
                mixpanel[name] = instances[name] = create_mplib(
                    token,
                    config,
                    name
                );
